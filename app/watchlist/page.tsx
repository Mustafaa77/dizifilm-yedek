...
function WatchlistPageContent() {
  ...
  useEffect(() => {
    const loadWatchlist = async () => {
      ...
      try {
        if (userData?.watchLaterItems && userData.watchLaterItems.length > 0) {
          const moviePromises = userData.watchLaterItems.map(id => fetchMovieById(parseInt(id)));
          const movies = await Promise.all(moviePromises);
          setWatchLaterMovies(movies.filter(Boolean) as TMDBMovie[]);
        }
      } catch (error) {
        console.error('İzlenecek listesi yüklenirken hata:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="container mx-auto px-4 py-12 space-y-12 animate-in fade-in duration-700">
      <div className="space-y-4">
        <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight flex items-center gap-4">
          ...
          <span className="text-gradient">İzlenecekler</span>
        </h1>
        <p className="text-muted-foreground font-medium">
          {watchLaterMovies.length > 0 ? `${watchLaterMovies.length} içerik izlenmeyi bekliyor` : 'Daha sonra izlemek için kaydettiğiniz her şey burada'}
        </p>
      </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-6 bg-muted/30 rounded-[3rem] border-2 border-dashed border-muted-foreground/10">
            ...
            <div className="space-y-2">
              <h3 className="text-2xl font-bold">Liste Henüz Boş</h3>
              <p className="text-muted-foreground max-w-sm">
                İzlemeye vakit bulamadığınız içerikleri bookmark ikonuna tıklayarak buraya ekleyebilirsiniz.
              </p>
            </div>
