...
export default function TVShowDetailPage() {
  ...
  const tvId = typeof params?.id === 'string' ? params.id : Array.isArray(params?.id) ? params?.id[0] : '';
          ...
          <section className="space-y-6">
            ...
            {credits?.cast?.length ? (
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
                {credits.cast.slice(0, 10).map((person: TMDBCastMember) => (
                  <div
                    key={person.id}
                  ...
                  >
                    <div className="relative h-20 w-20 rounded-full overflow-hidden ring-2 ring-transparent group-hover:ring-primary/30 transition-all bg-muted">
                      {person.profile_path ? (
                        <Image
                    ...
                    <div className="space-y-0.5 min-h-[3.5rem]">
                      <h4 className="text-xs sm:text-sm font-bold leading-tight line-clamp-2">{person.name}</h4>
      ...
      {similar?.results?.length ? (
        <section className="mt-16 space-y-8">
          <div className="flex items-center justify-between">
            ...
            <Button
              variant="ghost"
              onClick={() => router.push('/search?type=similar&tvId=' + tvId)}
